...
public class RandomDragTagView extends LinearLayout {
    ...
    // 是否显示左侧视图  默认显示左侧视图
    public boolean mIsShowLeftView = true;
    ...
    // 暴露接口
    public boolean mStartDrag = false;
    private OnRandomDragListener mDragListener;
    String  img;
    private FrameLayout leftFrame;
    private FrameLayout rightFrame;
    private ImageView rightDis;
    private ImageView leftDis;
    private onClickLinstener onClick;
   int id;
    public RandomDragTagView(Context context,int id ,String img) {
        super(context);
        this.img = img;
        this.id=id;
        setOrientation(HORIZONTAL);
        View inflate = inflate(context, R.layout.random_tag_layout, this);
        Log.i("TAG", "RandomDragTagView: "+img);
        ImageView img2 = inflate.findViewById(R.id.left_iv_tag);
        ImageView img3= inflate.findViewById(R.id.right_iv_tag);
        Glide.with(this).load(img).into(img2);
        Glide.with(this).load(img).into(img3);
        initView();
        initListener();
        initData();
    ...
    public RandomDragTagView(Context context,int id ,String img, @Nullable AttributeSet attrs) {
    ...
    public RandomDragTagView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
      ...
      /*  setOrientation(HORIZONTAL);
        View inflate = inflate(context, R.layout.random_tag_layout, this);
        ImageView img = inflate.findViewById(R.id.left_iv_tag);
    ...
    public void getMaxTextLayoutWidth() {
        post(new Runnable() {
            @Override
            public void run() {
                mMaxTextLayoutWidth = isShowLeftView() ? leftFrame.getWidth() : rightFrame.getWidth();
            }
        });
    }

    private void visibilityLeftLayout() {
        leftFrame.setVisibility(mIsShowLeftView ? VISIBLE : GONE);
    ...
    private void visibilityRightLayout() {
        rightFrame.setVisibility(!mIsShowLeftView ? VISIBLE : GONE);
    ...
    private void initView() {
        ...
        rightDis = findViewById(R.id.right_dis);
        leftDis = findViewById(R.id.left_dis);
        mLeftLine = findViewById(R.id.left_line_view);
        leftFrame = findViewById(R.id.left_tag_frame);
        mBreathingView = findViewById(R.id.white_breathing_view);
        mBreathingLayout = findViewById(R.id.fl_breathing);
        rightFrame = findViewById(R.id.right_tag_frame);
        ...
        leftDis.setOnClickListener(new OnClickListener() {
            @Override
        ...
        rightDis.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                if(onClick!=null){
                    onClick.onClick(id);
    ...
    public void switchDirection() {
        ...
        LayoutParams lp = (LayoutParams) (isShowLeftView() ?
                leftFrame.getLayoutParams() : rightFrame.getLayoutParams());
    ...
    private void handlerMoveEvent(float yDiff, float xDiff) {
        ...
        if (translationX <= 0) {
            ...
            int textWidth = isShowLeftView() ? leftFrame.getWidth() : rightFrame.getWidth();
            // 左侧视图
            if (isShowLeftView()) {
