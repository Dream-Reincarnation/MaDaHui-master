...
import com.ajiani.maidahui.Utils.anmion.ExpandAnimation;
...
import com.ajiani.maidahui.adapter.dynamic.MusicRelAdapter;
...
public class MusicRecommendFragment extends BaseFragment<SearMusic.searchView, SearMusicPresenter> implements SearMusic.searchView {
    ...
    public void close() {
        if (indext >= 0) {
            View childAt = musicRel.getChildAt(indext);
            MusicAdapter.ViewHolder childViewHolder = (MusicAdapter.ViewHolder) musicRel.getChildViewHolder(childAt);
            musicAdapter.close(childViewHolder);
        }
